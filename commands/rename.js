...
exports.run = (client, message, args, customisation) => {
  ...
  if (!mention){
    ...
    if (!user) return message.reply('You must tag someone, or give me a valid User ID for me to rename them.').catch(console.error);
  }else{
    user = message.guild.member(mention)
  }
  if (user.id === customisation.ownerid && message.author.id !== customisation.ownerid) return message.reply("You can't rename my developer, lol.");
