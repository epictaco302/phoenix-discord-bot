...
exports.run = (client, message, args, customisation) => {
  ...
  if (userRolePossition <= rolePosition) return message.channel.send("❌ I cannot ban that member because they have roles that is higher or equal to you.")
  if (botRolePossition <= rolePosition) return message.channel.send("❌ I cannot ban that member because they have roles that is higher or equal to me.")
  if (!message.guild.member(user).bannable) {
    message.channel.send(`:x: I cannot ban that member. My role might not be high enough or it's an internal error.`);
    return
  }else{
