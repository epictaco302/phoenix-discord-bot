...
exports.run = async (client, message, args, customisation) => {
  ...
  if(message.mentions.users.first().id === customisation.ownerid) return message.reply('You can\'t mute him you dingus.')
  if(message.author.id === message.mentions.users.first()) return message.reply("You can't mute yourself... Do you need to go back to 1st grade?");
  if (!muteRole) {
    try {
  ...
  if (message.guild.member(user).roles.cache.has(muteRole.id)) {
    if(message.content.includes("tb!mute")) return message.reply("That user has already been muted.")
    ...
    if(message.content.includes("tb!unmute")) return message.reply("that user has not been muted **yet**")
    message.guild.member(user).roles.add(muteRole).then(() => {
      ...
      .addField('User:', `${user.username}#${user.discriminator} (${user.id})`)
      .addField('Moderator:', `${message.author.username}#${message.author.discriminator}`)
      .addField('Reason', reason)      let logchannel = message.guild.channels.cache.find(x => x.name = 'logs');
      if  (!logchannel){
      message.channel.send({embed})
      }else{
        client.channels.cache.get(logchannel.id).send({embed});
