...
exports.run = (client, message, args) => {
  ...
  if (!message.member.hasPermission("MANAGE_CHANNELS")) return msg.reply("❌ You don't have the permission to do that!");
  ...
  if (validUnlocks.includes(time)) {
    ...
    message.channel.createOverwrite(message.guild.id, {
      ...
      message.channel.send(`Damn, **${message.author.username}** just locked the channel down for ${ms(ms(time), { long:true })}`).then(() => {
