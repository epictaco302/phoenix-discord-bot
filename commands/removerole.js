exports.run = (client, message, args, customisation) => {
    if (!message.guild.member(client.user).hasPermission("MANAGE_ROLES")) return message.reply("❌ I don't have the **Manage Roles** permission!");
    ...
    if (userRolePossition <= rolePosition) return message.channel.send("❌ Failed to add the role to the user because your role is lower than the specified role.")
    if (botRolePosition <= rolePosition) return message.channel.send("❌ Failed to add the role to the user because my highest role is lower than the specified role.");
    member.roles.remove(role).catch(e => {
        return message.channel.send("❌ There was an error! It most likely is that the role you are trying to remove is higher than the the role I have!");
    });
