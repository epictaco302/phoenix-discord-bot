...
exports.run = async (client, message, args, customisation) => {
  ...
  if (!message.member.hasPermission("KICK_MEMBERS")) return message.reply("❌ You don't have the **Kick Members** permission!");
  if (message.mentions.users.size < 1) return message.reply('You must mention someone to warn them.').catch(console.error);
  if (message.mentions.users.first().id === message.author.id) return message.reply('I can\' let you do that, self-harm is bad!');
  if (message.mentions.users.first().id === customisation.ownerid) return message.reply("You can't warn my developer, lol.");
  ...
  if(warns[`${user.id}, ${message.guild.id}`].warns == 3){
    ...
    message.reply('That person got kicked. Follow the rules, baka!')
  }

  if(warns[`${user.id}, ${message.guild.id}`].warns == 5){
    message.guild.member(user).ban(reason);
    message.reply('You won\'t have to worry about that shit-head any longer, I banned them!');
  }
